    ...
    $scope.ApplyKey = function (sender) {
         var IsValid = true;
            ...
            if ($scope.ActivationKey.trim().replace(/-/g, '').length == 32) {
                common.webApi.post('~activation/activate', 'appname=' + CommonSvc.appName + '&key=' + $scope.ActivationKey).success(function (success) {
             ...
             var IsValid = true;
                ...
                SweetAlert.swal({
                    ...
                    if (isConfirm) {
                        ...
                        common.webApi.post('~activation/availableeditions', 'appname=' + CommonSvc.appName + '&edition=' + $scope.Edition + '&email=' + $scope.RequestEmail).success(function (success) {
