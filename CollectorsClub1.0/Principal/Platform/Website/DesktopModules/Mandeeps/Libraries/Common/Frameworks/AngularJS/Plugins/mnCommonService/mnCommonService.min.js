...
mnSvc.factory('CommonSvc', function ($rootScope, SweetAlert) {
    ...
    var initData = function ($rootScope, $scope, $attrs, $http) {
        ...
        var common = ___WebAPI.Angular($scope.moduleid, app.name, $http);
...
mnSvc.directive('uiengine', ['$compile', '$timeout', 'CommonSvc', '$routeParams', '$rootScope', function ($compile, $timeout, CommonSvc, $routeParams, $rootScope) {
    var linker = function ($scope, $element, $attrs) {
        ...
        var uienginepath = window[app.name + '_UIEngine' + $attrs.provider + 'Path'];
        var uitemplatepath = window[app.name + '_UITemplatePath'];
        ...
        $scope.PreviewImport = function (IsSaveCall) {
            ...
            if (ValidateMappings(allControls)) {
                ...
                if (!IsSaveCall) {
                    ...
                    common.webApi.post('' + $scope.ui.data.Controller.Value + '/bulkadd', '', dImport).success(function (data) {
        ...
        $scope.ManageSlug = function (eid) {
            if (eid != undefined) {
                var popUpWindow = window.open("about:blank", "", "height=700px,width=1000px");
                common.webApi.get('ui/getlink', 'identifier=' + $attrs.identifier + '&entityid=' + eid).success(function (data) {
                    if (data.length > 0)
                        popUpWindow.location.href = data;
                })
            }
        };
