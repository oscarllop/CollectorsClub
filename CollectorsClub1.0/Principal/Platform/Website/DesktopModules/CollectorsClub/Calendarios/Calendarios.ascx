<%@ Control Language="C#" AutoEventWireup="true" Inherits="CollectorsClub.UI.Skins.Controls.Calendarios" CodeFile="Calendarios.ascx.cs" %>
<asp:panel runat="server" id="Container">
	...
	<div id="CalendariosPrincipalContenedorCargando" class="ContenedorCargando" style="height: 50px; width: 50px;" data-elementoReferencia="CalendariosPrincipalContenedorExterno"></div>
	<div id="CalendariosPrincipalContenedorExterno" style="display: block;" data-bind="partialbieniniciada: Parametros"></div>
	<script type="text/javascript">
		if (typeof window.ParametrosGlobales === 'undefined') {
			window.ParametrosGlobales = {
				...
				IdCalendario: '<%= Session["IdCalendario"] %>',
		...
		require(['ParametrosGlobales', 'use!porto/Resources/Menu/menu.min', 'knockout', 'Globales/knockout.extensions', 'dojox/json/ref', 'Globales/Cargando', 'WebAPI/knockout.mapping-latest', 'jquery', 'Globales/Error', 'DNN/dnn.modalpopup', 'Globales/popup', 'Globales/Core.DoubleScroll', 'Globales/Core.ExpandToParent', 'Globales/Core.Utils', 'Plugins/inputmask/inputmask', 'Plugins/inputmask/jquery.inputmask', 'Plugins/inputmask/inputmask.regex.extensions', 'Plugins/inputmask/inputmask.phone.extensions', 'Globales/partials'], function (parametrosGlobales, menu, ko, koextensions, dojoref, cargando, mapping, $, error, modalPopup, popup, doubleScroll, expandToParent, utils, inputmask, jqueryInputmask, inputmaskRegexExtension, inputmaskPhoneExtension, partials) {
			...
			try {
				var _parametros = {
					...
					IdModulo: 'Calendarios',
					Entidad: 'Calendario',
					IdPartial: 'CalendariosPrincipal',
					Contenedor: '#CalendariosPrincipalContenedorInterno',
					TamanyoPagina: 50,
					ColumnaOrden: 'Id',
					TipoOrden: 0,
				...
				var _parametrosUrl = ['IdDesde', 'IdHasta', 'IdCategoria', 'IdSubcategoria', 'Visible', 'Imagen', 'Nombre', 'IdEstado', 'Anyo', 'IdEntidad', 'Serie', 'NumeroRepetidosDesde', 'NumeroRepetidosHasta', 'NumeroSerie', 'IdEntidadContratante', 'DL', 'Variante', 'IdFabricante', 'PaginaWeb', 'Codigo', 'IdUsuario', 'Cultura', 'IdMarca'];
				var _filtro = {};
				...
				ko.applyBindings({ Parametros: _parametros }, $('#CalendariosPrincipalContenedorExterno')[0]);
