define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/FotoController', 'WebAPI/Models/FotoModel', 'WebAPI/Controllers/CategoriaFotoController', 'WebAPI/Models/CategoriaFotoModel', 'WebAPI/Controllers/MarcaController', 'WebAPI/Models/MarcaModel', 'WebAPI/Models/DocumentoUsuariosModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, FotoController, FotoModel, CategoriaFotoController, CategoriaFotoModel, MarcaController, MarcaModel, DocumentoUsuariosModel, utils, popup, moment) {
	try {
		return function FotoEditorViewModel(settings) {
			...
			self.BorradoLogicoDeArchivosCargaMasiva = (typeof (self.settings.BorradoLogicoDeArchivos) !== 'undefined' ? self.settings.BorradoLogicoDeArchivos : false);
			self.UrlArchivosCargaMasiva = ko.computed(function () {
				return self.settings.ApplicationPath + 'Portals/0/_extension/Imagenes/Fotos/';
			}); 
			self.NumeroMaximoArchivosACargarCargaMasiva = 50;

			self.CargaFinalizadaCargaMasiva = function (archivos) {
				self.DocumentosUsuarioCargaMasiva([]);
			}

			self.DocumentosUsuarioCargaMasiva = ko.observableArray([]);
			self.SincronizandoCargaMasiva = false;
			self.ActivarPestanyaFotoEditorCargaMasiva = ko.observable();
			self.ArchivosCargaMasiva = ko.observableArray([]);
			self.DocumentosUsuarioCargaMasiva.subscribe(function (documentos) {
				if (!self.SincronizandoCargaMasiva) {
					self.SincronizandoCargaMasiva = true;
					self.ArchivosCargaMasiva(documentos.map(function (documento) { return { Ruta: self.UrlArchivosCargaMasiva() + ko.unwrap(documento.RutaArchivo), Doctype: ko.unwrap(documento.Doctype) }; }));
					self.SincronizandoCargaMasiva = false;
				}
			}, self);

			self.ArchivosCargaMasiva.subscribe(function (archivos) {
				if (!self.SincronizandoCargaMasiva) {
					self.SincronizandoCargaMasiva = true;
					var _documentosActuales = self.DocumentosUsuarioCargaMasiva();
					var _documentosABorrar = new Array();

					$.each(_documentosActuales, function (indice, documento) {
						if (archivos.filter(function (archivo) { return self.UrlArchivosCargaMasiva() + ko.unwrap(documento.RutaArchivo) == archivo.Ruta; }).length == 0) {
							if (self.BorradoLogicoDeArchivosCargaMasiva) {
								var _fechaEliminacion = (typeof moment !== 'undefined' ? moment().format() : new Date('01/01/0001 0:00:00'));
								if (ko.isObservable(documento.FechaEliminacion)) { documento.FechaEliminacion(_fechaEliminacion); } else { documento.FechaEliminacion = _fechaEliminacion; }
							} else {
								_documentosABorrar.push(indice);
					...
					$.each(archivos, function (indice, archivo) {
						// TODO: OLL: Mirar si es necesario comparar más propiedades para que siempre funcione. Lo ideal sería que archivo tuviese también el Id de documento, así podríamos por Id reconocer mejor si el elemento estaba en 
						// DocumentosUsuario o no
						if (_documentosActuales.filter(function (documento) { return self.UrlArchivosCargaMasiva() + ko.unwrap(documento.RutaArchivo) == archivo.Ruta; }).length == 0) {
							//TODO: OLL: Falta pedir y añadir el tipo correctamente.
							var _partesRuta = archivo.Ruta.split('/');
							_documentosActuales.push(new DocumentoUsuariosModel({ Id: --self.IdDocumentoUsuarioModelNuevo, Nombre: _partesRuta[_partesRuta.length - 1], RutaArchivo: archivo.Ruta.replace(self.UrlArchivosCargaMasiva(), ''), Doctype: archivo.Doctype, IdMarca: self.settings.IdMarca, IdTipo: 1, UsuarioAlta: (typeof (settings.Usuario) !== 'undefined' ? settings.Usuario : 'usuario_anonimo') }));
						}
					});
					for (var i = 0; i < _documentosABorrar.length; i++) {
						_documentosActuales = _documentosActuales.filter(function (registro) { return registro.Id !== _documentosActuales[_documentosABorrar[i]].Id; });
					}
					self.DocumentosUsuarioCargaMasiva(_documentosActuales);
					self.SincronizandoCargaMasiva = false;
				}
			}, self);

			self.GrupoArchivosAnyadidoCargaMasiva = function (gestorArchivos) {
				var _respuesta = confirm('Atención!!! Se van a cargar todas las imágenes del archivo comprimido. Este proceso sobreescribe cualquier imagen que exista. Verifique que ha seleccionado correctamente la edición y ha subido el archivo adecuado antes de continuar.')
				if (_respuesta) {
					utils.Cargando.Mostrar(settings.IdPartial + 'ContenedorCargando');
				}
				return _respuesta;
			}

			self.CargaFinalizadaCargaMasiva = function (respuesta) {
				utils.Cargando.Ocultar(settings.IdPartial + 'ContenedorCargando');
			}

			self.ErrorCargaCargaMasiva = function (respuesta) {
				utils.Cargando.Ocultar(settings.IdPartial + 'ContenedorCargando');
			}
			// #endregion Carga masiva
			...
			self.UrlArchivosNombreArchivoImagen = ko.computed(function () {
				return (self.Registro() != null ? self.settings.ApplicationPath + 'Portals/0/_extension/Imagenes/Fotos' : '');
			});

			self.UrlArchivosNuevosNombreArchivoImagen = ko.computed(function () {
				var _nombreCategoria = '';
				if (self.Registro() != null && ko.unwrap(self.Registro().IdCategoria) != null) {
					var _idCategoria = ko.unwrap(self.Registro().IdCategoria);
					$(self.CategoriasFotos()).each(function (indice) { if (this.Id == _idCategoria) { _nombreCategoria = this.Nombre; }; })
				}
				return self.UrlArchivosNombreArchivoImagen() + '/' + _nombreCategoria;
			});

			self.UrlArchivosNuevosCargaMasiva = ko.computed(function () {
				return 'Portals/0/_extension/Imagenes/Fotos/'
			});

			self.ElementosPorGrupo = 3;
			self.GruposRegistros = ko.computed(function () {
			...
			self.GruposRegistrosFijos = ko.computed(function () {
				var _grupos = new Array();
				$(self.RegistrosFijos()).each(function (indice) {
					if (_grupos.length == 0 || _grupos.length > 0 && _grupos[_grupos.length - 1].Registros().length == self.ElementosPorGrupo) { _grupos.push({ Registros: ko.observableArray([]) }); }
					if (_grupos[_grupos.length - 1].Registros().length < self.ElementosPorGrupo) { _grupos[_grupos.length - 1].Registros().push(this); }
				});
				return _grupos
