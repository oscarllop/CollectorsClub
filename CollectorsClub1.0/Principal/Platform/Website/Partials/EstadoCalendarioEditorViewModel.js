define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/EstadoCalendarioController', 'WebAPI/Models/EstadoCalendarioModel', 'WebAPI/Controllers/MarcaController', 'WebAPI/Models/MarcaModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, EstadoCalendarioController, EstadoCalendarioModel, MarcaController, MarcaModel, utils, popup, moment) {
	try {
		return function EstadoCalendarioEditorViewModel(settings) {
			var self = this;
			self.settings = $.extend({ Multiidioma: true }, settings);
			// #region Propiedades y métodos que difieren por entidad
			self.Entidad = 'EstadoCalendario';
			...
			self.MetodoObtenerPorFiltros = 'EstadoCalendario_ObtenerPorFiltros';
			self.MetodoObtenerCantidadPorFiltros = 'EstadoCalendario_ObtenerCantidadPorFiltros';
			...
			self.New = function () { baseViewModel.New(self, EstadoCalendarioModel); };
			...
			self.Inicializar = function (settings) { baseViewModel.Inicializar(self, settings, EstadoCalendarioModel); };
			...
			self.MostrarDialogoCopia = function (viewModel) { baseViewModel.MostrarDialogoCopia(viewModel, EstadoCalendarioModel); };
			...
			self.MostrarDialogoEdicionMultiple = function (viewModel) { baseViewModel.MostrarDialogoEdicionMultiple(viewModel, EstadoCalendarioModel); };
			...
			if (typeof (GlobalControllers) != 'undefined' && typeof (GlobalControllers['EstadoCalendarioController']) == 'undefined') { GlobalControllers['EstadoCalendarioController'] = new EstadoCalendarioController(settings); }
			...
			baseViewModel.InicializarViewModel(self, settings, EstadoCalendarioModel);
		};
	} catch (excepcion) {
