	...
	$.mbExtruder= {
		...
		openMbExtruder:function(c){
			...
			if (position=="top" || position=="bottom"){
				extruder.find('.extruder-content').slideDown(opt.slideTimer, function () { if (opt.onExtFinishOpen) opt.onExtFinishOpen(); });
				...
				extruder.find('.extruder-content').animate({ width: opt.width}, opt.slideTimer,function(){
					...
					if (opt.onExtFinishOpen) opt.onExtFinishOpen();
				});
				if(opt.onExtOpen) opt.onExtOpen();
			}
		...
		closeMbExtruder:function(){
			...
			if (opt.position=="top" || opt.position=="bottom"){
				extruder.find('.extruder-content').slideUp(opt.slideTimer, function () { if (opt.onExtFinishClose) opt.onExtFinishClose(); });
				...
				extruder.find('.extruder-content').animate({ width: 1 }, opt.slideTimer,function(){
					...
					if (opt.onExtFinishClose) opt.onExtFinishClose();
				});
				if(opt.onExtClose) opt.onExtClose();
			}
		}
