define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/TipoColeccionCalendarioController', 'WebAPI/Models/TipoColeccionCalendarioModel', 'WebAPI/Controllers/MarcaController', 'WebAPI/Models/MarcaModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, TipoColeccionCalendarioController, TipoColeccionCalendarioModel, MarcaController, MarcaModel, utils, popup, moment) {
	try {
		return function TipoColeccionCalendarioEditorViewModel(settings) {
			var self = this;
			self.settings = $.extend({ Multiidioma: true }, settings);
			// #region Propiedades y métodos que difieren por entidad
			self.Entidad = 'TipoColeccionCalendario';
			...
			self.MetodoObtenerPorFiltros = 'TipoColeccionCalendario_ObtenerPorFiltros';
			self.MetodoObtenerCantidadPorFiltros = 'TipoColeccionCalendario_ObtenerCantidadPorFiltros';
			...
			self.New = function () { baseViewModel.New(self, TipoColeccionCalendarioModel); };
			...
			self.Inicializar = function (settings) { baseViewModel.Inicializar(self, settings, TipoColeccionCalendarioModel); };
			...
			self.MostrarDialogoCopia = function (viewModel) { baseViewModel.MostrarDialogoCopia(viewModel, TipoColeccionCalendarioModel); };
			...
			self.MostrarDialogoEdicionMultiple = function (viewModel) { baseViewModel.MostrarDialogoEdicionMultiple(viewModel, TipoColeccionCalendarioModel); };
			...
			if (typeof (GlobalControllers) != 'undefined' && typeof (GlobalControllers['TipoColeccionCalendarioController']) == 'undefined') { GlobalControllers['TipoColeccionCalendarioController'] = new TipoColeccionCalendarioController(settings); }
			...
			baseViewModel.InicializarViewModel(self, settings, TipoColeccionCalendarioModel);
		};
	} catch (excepcion) {
