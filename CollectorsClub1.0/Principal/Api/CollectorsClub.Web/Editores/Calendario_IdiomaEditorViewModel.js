define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/Calendario_IdiomaController', 'WebAPI/Models/Calendario_IdiomaModel', 'WebAPI/Controllers/CalendarioController', 'WebAPI/Models/CalendarioModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, Calendario_IdiomaController, Calendario_IdiomaModel, CalendarioController, CalendarioModel, utils, popup, moment) {
	try {
		return function Calendario_IdiomaEditorViewModel(settings) {
			var self = this;
			self.settings = $.extend({}, settings);
			// #region Propiedades y métodos que difieren por entidad
			self.Entidad = 'Calendario_Idioma';
			self.CamposVisiblesInsercion = { Id: true, IdRegistro: true, Cultura: true, Nombre: true, Variante: true };
			self.CamposVisiblesEdicion = { Id: true, IdRegistro: true, Cultura: true, Nombre: true, Variante: true };
			self.CamposHabilitadosInsercion = { Id: false, IdRegistro: true, Cultura: true, Nombre: true, Variante: true };
			self.CamposHabilitadosEdicion = { Id: false, IdRegistro: true, Cultura: true, Nombre: true, Variante: true };
			self.CamposHabilitadosFiltro = { Id: true, IdRegistro: true, Cultura: true, Nombre: true, Variante: true };
			...
			self.EntidadesRelacionadas = ['Calendario'];
			...
			self.MetodoObtenerPorFiltros = 'Calendario_Idioma_ObtenerPorFiltros';
			self.MetodoObtenerCantidadPorFiltros = 'Calendario_Idioma_ObtenerCantidadPorFiltros';
			...
			self.New = function () { baseViewModel.New(self, Calendario_IdiomaModel); };
			...
			self.Inicializar = function (settings) { baseViewModel.Inicializar(self, settings, Calendario_IdiomaModel); };
			...
			self.MostrarDialogoCopia = function (viewModel) { baseViewModel.MostrarDialogoCopia(viewModel, Calendario_IdiomaModel); };
			...
			self.MostrarDialogoEdicionMultiple = function (viewModel) { baseViewModel.MostrarDialogoEdicionMultiple(viewModel, Calendario_IdiomaModel); };
			...
			if (typeof (GlobalControllers) != 'undefined' && typeof (GlobalControllers['Calendario_IdiomaController']) == 'undefined') { GlobalControllers['Calendario_IdiomaController'] = new Calendario_IdiomaController(settings); }
			...
			self.Calendarios = ko.lazyObservableArray(GlobalControllers['CalendarioController'].GetQuery, self, {
				Todos: true,
				Parameters: 'query=Calendario_ObtenerComboPorIdMarca' +
			...
			baseViewModel.InicializarViewModel(self, settings, Calendario_IdiomaModel);
		};
	} catch (excepcion) {
