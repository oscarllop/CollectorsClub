define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/CategoriaCalendarioController', 'WebAPI/Models/CategoriaCalendarioModel', 'WebAPI/Controllers/MarcaController', 'WebAPI/Models/MarcaModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, CategoriaCalendarioController, CategoriaCalendarioModel, MarcaController, MarcaModel, utils, popup, moment) {
	try {
		return function CategoriaCalendarioEditorViewModel(settings) {
			var self = this;
			self.settings = $.extend({ Multiidioma: true }, settings);
			// #region Propiedades y métodos que difieren por entidad
			self.Entidad = 'CategoriaCalendario';
			...
			self.MetodoObtenerPorFiltros = 'CategoriaCalendario_ObtenerPorFiltros';
			self.MetodoObtenerCantidadPorFiltros = 'CategoriaCalendario_ObtenerCantidadPorFiltros';
			...
			self.New = function () { baseViewModel.New(self, CategoriaCalendarioModel); };
			...
			self.Inicializar = function (settings) { baseViewModel.Inicializar(self, settings, CategoriaCalendarioModel); };
			...
			self.MostrarDialogoCopia = function (viewModel) { baseViewModel.MostrarDialogoCopia(viewModel, CategoriaCalendarioModel); };
			...
			self.MostrarDialogoEdicionMultiple = function (viewModel) { baseViewModel.MostrarDialogoEdicionMultiple(viewModel, CategoriaCalendarioModel); };
			...
			if (typeof (GlobalControllers) != 'undefined' && typeof (GlobalControllers['CategoriaCalendarioController']) == 'undefined') { GlobalControllers['CategoriaCalendarioController'] = new CategoriaCalendarioController(settings); }
			...
			baseViewModel.InicializarViewModel(self, settings, CategoriaCalendarioModel);
		};
	} catch (excepcion) {
