define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/FabricanteController', 'WebAPI/Models/FabricanteModel', 'WebAPI/Controllers/MarcaController', 'WebAPI/Models/MarcaModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, FabricanteController, FabricanteModel, MarcaController, MarcaModel, utils, popup, moment) {
	try {
		return function FabricanteEditorViewModel(settings) {
			var self = this;
			self.settings = $.extend({ Multiidioma: true }, settings);
			// #region Propiedades y métodos que difieren por entidad
			self.Entidad = 'Fabricante';
			...
			self.MetodoObtenerPorFiltros = 'Fabricante_ObtenerPorFiltros';
			self.MetodoObtenerCantidadPorFiltros = 'Fabricante_ObtenerCantidadPorFiltros';
			...
			self.New = function () { baseViewModel.New(self, FabricanteModel); };
			...
			self.Inicializar = function (settings) { baseViewModel.Inicializar(self, settings, FabricanteModel); };
			...
			self.MostrarDialogoCopia = function (viewModel) { baseViewModel.MostrarDialogoCopia(viewModel, FabricanteModel); };
			...
			self.MostrarDialogoEdicionMultiple = function (viewModel) { baseViewModel.MostrarDialogoEdicionMultiple(viewModel, FabricanteModel); };
			...
			if (typeof (GlobalControllers) != 'undefined' && typeof (GlobalControllers['FabricanteController']) == 'undefined') { GlobalControllers['FabricanteController'] = new FabricanteController(settings); }
			...
			baseViewModel.InicializarViewModel(self, settings, FabricanteModel);
		};
	} catch (excepcion) {
