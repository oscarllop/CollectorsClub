define(['Globales/BaseViewModelEditor', 'knockout', 'global/knockout/knockout.validation.min', 'Globales/ko.observableDictionary', 'WebAPI/Controllers/Fabricante_IdiomaController', 'WebAPI/Models/Fabricante_IdiomaModel', 'WebAPI/Controllers/FabricanteController', 'WebAPI/Models/FabricanteModel', 'Globales/Core.Utils', 'Globales/popup', 'moment/moment-with-locales.min'], function (baseViewModel, ko, validation, observableDictionary, Fabricante_IdiomaController, Fabricante_IdiomaModel, FabricanteController, FabricanteModel, utils, popup, moment) {
	try {
		return function Fabricante_IdiomaEditorViewModel(settings) {
			var self = this;
			self.settings = $.extend({}, settings);
			// #region Propiedades y métodos que difieren por entidad
			self.Entidad = 'Fabricante_Idioma';
			...
			self.EntidadesRelacionadas = ['Fabricante'];
			...
			self.MetodoObtenerPorFiltros = 'Fabricante_Idioma_ObtenerPorFiltros';
			self.MetodoObtenerCantidadPorFiltros = 'Fabricante_Idioma_ObtenerCantidadPorFiltros';
			...
			self.New = function () { baseViewModel.New(self, Fabricante_IdiomaModel); };
			...
			self.Inicializar = function (settings) { baseViewModel.Inicializar(self, settings, Fabricante_IdiomaModel); };
			...
			self.MostrarDialogoCopia = function (viewModel) { baseViewModel.MostrarDialogoCopia(viewModel, Fabricante_IdiomaModel); };
			...
			self.MostrarDialogoEdicionMultiple = function (viewModel) { baseViewModel.MostrarDialogoEdicionMultiple(viewModel, Fabricante_IdiomaModel); };
			...
			if (typeof (GlobalControllers) != 'undefined' && typeof (GlobalControllers['Fabricante_IdiomaController']) == 'undefined') { GlobalControllers['Fabricante_IdiomaController'] = new Fabricante_IdiomaController(settings); }
			...
			baseViewModel.InicializarViewModel(self, settings, Fabricante_IdiomaModel);
		};
	} catch (excepcion) {
