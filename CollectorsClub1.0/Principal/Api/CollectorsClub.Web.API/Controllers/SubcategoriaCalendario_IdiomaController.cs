...
namespace CollectorsClub.Web.API.Controllers {
	...
	[Authorize]
	public partial class SubcategoriaCalendario_IdiomaController : ApiController {
	  ...
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		...
		public SubcategoriaCalendario_IdiomaController(ICommandBus commandBus, ISubcategoriaCalendario_IdiomaRepository subcategoriacalendario_idiomaRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<SubcategoriaCalendario_Idioma>>(HttpStatusCode.OK, subcategoriacalendario_idiomaRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				SubcategoriaCalendario_Idioma subcategoriacalendario_idioma = subcategoriacalendario_idiomaRepository.GetById(include, (p => p.Id == Id));
				if (subcategoriacalendario_idioma == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
				return Request.CreateResponse<SubcategoriaCalendario_Idioma>(HttpStatusCode.OK, subcategoriacalendario_idioma, formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<SubcategoriaCalendario_Idioma>>(HttpStatusCode.OK, subcategoriacalendario_idiomaRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, subcategoriacalendario_idiomaRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Post(SubcategoriaCalendario_IdiomaModel subcategoriacalendario_idioma) {
			try {
				if (ModelState.IsValid) {
					if (subcategoriacalendario_idioma.Id == 0) {
						var command = AutoMapper.Mapper.Map<SubcategoriaCalendario_IdiomaModel, CreateOrUpdateSubcategoriaCalendario_IdiomaCommand>(subcategoriacalendario_idioma);
						var result = commandBus.Submit(command);
						if (result.Success) {
							subcategoriacalendario_idioma = AutoMapper.Mapper.Map<CreateOrUpdateSubcategoriaCalendario_IdiomaCommand, SubcategoriaCalendario_IdiomaModel>(command);
							var response = Request.CreateResponse<SubcategoriaCalendario_IdiomaModel>(HttpStatusCode.Created, subcategoriacalendario_idioma);
							string uri = Url.Route(null, new { Id = subcategoriacalendario_idioma.Id });
		...
		public HttpResponseMessage Put(int Id, SubcategoriaCalendario_IdiomaModel subcategoriacalendario_idioma) {
			try {
				if (ModelState.IsValid) {
					var command = AutoMapper.Mapper.Map<SubcategoriaCalendario_IdiomaModel, CreateOrUpdateSubcategoriaCalendario_IdiomaCommand>(subcategoriacalendario_idioma);
					var result = commandBus.Submit(command);
					return Request.CreateResponse<SubcategoriaCalendario_IdiomaModel>(HttpStatusCode.OK, subcategoriacalendario_idioma);
				} else {
					var errors = new Dictionary<string, IEnumerable<string>>();
					foreach (var keyValue in ModelState) {
							errors[keyValue.Key] = keyValue.Value.Errors.Select(e => (!string.IsNullOrWhiteSpace(e.ErrorMessage) ? e.ErrorMessage : (e.Exception != null ? e.Exception.Message : string.Empty)));
		...
		public HttpResponseMessage Delete(int Id) {
			try {
				var command = new DeleteSubcategoriaCalendario_IdiomaCommand(Id);
				var result = commandBus.Submit(command);
