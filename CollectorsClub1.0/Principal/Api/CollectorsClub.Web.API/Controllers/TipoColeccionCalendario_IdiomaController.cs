...
namespace CollectorsClub.Web.API.Controllers {
	...
	[Authorize]
	public partial class TipoColeccionCalendario_IdiomaController : ApiController {
	  ...
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		...
		public TipoColeccionCalendario_IdiomaController(ICommandBus commandBus, ITipoColeccionCalendario_IdiomaRepository tipocoleccioncalendario_idiomaRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<TipoColeccionCalendario_Idioma>>(HttpStatusCode.OK, tipocoleccioncalendario_idiomaRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				TipoColeccionCalendario_Idioma tipocoleccioncalendario_idioma = tipocoleccioncalendario_idiomaRepository.GetById(include, (p => p.Id == Id));
				if (tipocoleccioncalendario_idioma == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
				return Request.CreateResponse<TipoColeccionCalendario_Idioma>(HttpStatusCode.OK, tipocoleccioncalendario_idioma, formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<TipoColeccionCalendario_Idioma>>(HttpStatusCode.OK, tipocoleccioncalendario_idiomaRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, tipocoleccioncalendario_idiomaRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Post(TipoColeccionCalendario_IdiomaModel tipocoleccioncalendario_idioma) {
			try {
				if (ModelState.IsValid) {
					if (tipocoleccioncalendario_idioma.Id == 0) {
						var command = AutoMapper.Mapper.Map<TipoColeccionCalendario_IdiomaModel, CreateOrUpdateTipoColeccionCalendario_IdiomaCommand>(tipocoleccioncalendario_idioma);
						var result = commandBus.Submit(command);
						if (result.Success) {
							tipocoleccioncalendario_idioma = AutoMapper.Mapper.Map<CreateOrUpdateTipoColeccionCalendario_IdiomaCommand, TipoColeccionCalendario_IdiomaModel>(command);
							var response = Request.CreateResponse<TipoColeccionCalendario_IdiomaModel>(HttpStatusCode.Created, tipocoleccioncalendario_idioma);
							string uri = Url.Route(null, new { Id = tipocoleccioncalendario_idioma.Id });
		...
		public HttpResponseMessage Put(int Id, TipoColeccionCalendario_IdiomaModel tipocoleccioncalendario_idioma) {
			try {
				if (ModelState.IsValid) {
					var command = AutoMapper.Mapper.Map<TipoColeccionCalendario_IdiomaModel, CreateOrUpdateTipoColeccionCalendario_IdiomaCommand>(tipocoleccioncalendario_idioma);
					var result = commandBus.Submit(command);
					return Request.CreateResponse<TipoColeccionCalendario_IdiomaModel>(HttpStatusCode.OK, tipocoleccioncalendario_idioma);
				} else {
					var errors = new Dictionary<string, IEnumerable<string>>();
					foreach (var keyValue in ModelState) {
							errors[keyValue.Key] = keyValue.Value.Errors.Select(e => (!string.IsNullOrWhiteSpace(e.ErrorMessage) ? e.ErrorMessage : (e.Exception != null ? e.Exception.Message : string.Empty)));
		...
		public HttpResponseMessage Delete(int Id) {
			try {
				var command = new DeleteTipoColeccionCalendario_IdiomaCommand(Id);
				var result = commandBus.Submit(command);
