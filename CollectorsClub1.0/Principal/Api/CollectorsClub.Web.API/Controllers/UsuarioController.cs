...
namespace CollectorsClub.Web.API.Controllers {
	...
	public partial class UsuarioController : ApiController {
	  ...
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		...
		public UsuarioController(ICommandBus commandBus, IUsuarioRepository usuarioRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<Usuario>>(HttpStatusCode.OK, usuarioRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				Usuario usuario = usuarioRepository.GetById(include, (p => p.Id == Id));
				if (usuario == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
				return Request.CreateResponse<Usuario>(HttpStatusCode.OK, usuario, formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetPorMarca([FromUri] string idMarca, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<Usuario>>(HttpStatusCode.OK, usuarioRepository.GetMany(include, r => r.IdMarca == idMarca).AsQueryable(), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<Usuario>>(HttpStatusCode.OK, usuarioRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetPorFiltros(string idMarca, string nombre, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<Usuario>>(HttpStatusCode.OK, usuarioRepository.GetMany(r => r.IdMarca == idMarca && (nombre == null || r.Nombre.Contains(nombre))).AsQueryable(), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, usuarioRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Post(UsuarioModel usuario) {
			try {
				if (ModelState.IsValid) {
					if (usuario.Id == 0) {
						var command = AutoMapper.Mapper.Map<UsuarioModel, CreateOrUpdateUsuarioCommand>(usuario);
						var result = commandBus.Submit(command);
						if (result.Success) {
							usuario = AutoMapper.Mapper.Map<CreateOrUpdateUsuarioCommand, UsuarioModel>(command);
							var response = Request.CreateResponse<UsuarioModel>(HttpStatusCode.Created, usuario);
							string uri = Url.Route(null, new { Id = usuario.Id });
		...
		public HttpResponseMessage Put(int Id, UsuarioModel usuario) {
			try {
				if (ModelState.IsValid) {
					var command = AutoMapper.Mapper.Map<UsuarioModel, CreateOrUpdateUsuarioCommand>(usuario);
					var result = commandBus.Submit(command);
					return Request.CreateResponse<UsuarioModel>(HttpStatusCode.OK, usuario);
				} else {
					var errors = new Dictionary<string, IEnumerable<string>>();
					foreach (var keyValue in ModelState) {
							errors[keyValue.Key] = keyValue.Value.Errors.Select(e => (!string.IsNullOrWhiteSpace(e.ErrorMessage) ? e.ErrorMessage : (e.Exception != null ? e.Exception.Message : string.Empty)));
