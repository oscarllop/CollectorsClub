...
namespace CollectorsClub.Web.API.Controllers {
	...
	[Authorize]
	public partial class Fabricante_IdiomaController : ApiController {
		private readonly ICommandBus commandBus;
		private readonly IFabricante_IdiomaRepository fabricante_idiomaRepository;
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		
		public Fabricante_IdiomaController(ICommandBus commandBus, IFabricante_IdiomaRepository fabricante_idiomaRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<Fabricante_Idioma>>(HttpStatusCode.OK, fabricante_idiomaRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				if (fabricante_idioma == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<Fabricante_Idioma>>(HttpStatusCode.OK, fabricante_idiomaRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, fabricante_idiomaRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Post(Fabricante_IdiomaModel fabricante_idioma) {
			try {
				if (ModelState.IsValid) {
					if (fabricante_idioma.Id == 0) {
						var command = AutoMapper.Mapper.Map<Fabricante_IdiomaModel, CreateOrUpdateFabricante_IdiomaCommand>(fabricante_idioma);
						var result = commandBus.Submit(command);
						if (result.Success) {
							fabricante_idioma = AutoMapper.Mapper.Map<CreateOrUpdateFabricante_IdiomaCommand, Fabricante_IdiomaModel>(command);
							var response = Request.CreateResponse<Fabricante_IdiomaModel>(HttpStatusCode.Created, fabricante_idioma);
							string uri = Url.Route(null, new { Id = fabricante_idioma.Id });
		...
		public HttpResponseMessage Put(int Id, Fabricante_IdiomaModel fabricante_idioma) {
			try {
				if (ModelState.IsValid) {
					var command = AutoMapper.Mapper.Map<Fabricante_IdiomaModel, CreateOrUpdateFabricante_IdiomaCommand>(fabricante_idioma);
					var result = commandBus.Submit(command);
					return Request.CreateResponse<Fabricante_IdiomaModel>(HttpStatusCode.OK, fabricante_idioma);
				} else {
					var errors = new Dictionary<string, IEnumerable<string>>();
					foreach (var keyValue in ModelState) {
							errors[keyValue.Key] = keyValue.Value.Errors.Select(e => (!string.IsNullOrWhiteSpace(e.ErrorMessage) ? e.ErrorMessage : (e.Exception != null ? e.Exception.Message : string.Empty)));
		...
		public HttpResponseMessage Delete(int Id) {
			try {
				var command = new DeleteFabricante_IdiomaCommand(Id);
				var result = commandBus.Submit(command);
