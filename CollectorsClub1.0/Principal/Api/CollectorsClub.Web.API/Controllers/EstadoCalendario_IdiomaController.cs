...
namespace CollectorsClub.Web.API.Controllers {
	...
	[Authorize]
	public partial class EstadoCalendario_IdiomaController : ApiController {
		private readonly ICommandBus commandBus;
		private readonly IEstadoCalendario_IdiomaRepository estadocalendario_idiomaRepository;
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		
		public EstadoCalendario_IdiomaController(ICommandBus commandBus, IEstadoCalendario_IdiomaRepository estadocalendario_idiomaRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<EstadoCalendario_Idioma>>(HttpStatusCode.OK, estadocalendario_idiomaRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				EstadoCalendario_Idioma estadocalendario_idioma = estadocalendario_idiomaRepository.GetById(include, (p => p.Id == Id));
				if (estadocalendario_idioma == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
				return Request.CreateResponse<EstadoCalendario_Idioma>(HttpStatusCode.OK, estadocalendario_idioma, formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<EstadoCalendario_Idioma>>(HttpStatusCode.OK, estadocalendario_idiomaRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, estadocalendario_idiomaRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Delete(int Id) {
			try {
				var command = new DeleteEstadoCalendario_IdiomaCommand(Id);
				var result = commandBus.Submit(command);
