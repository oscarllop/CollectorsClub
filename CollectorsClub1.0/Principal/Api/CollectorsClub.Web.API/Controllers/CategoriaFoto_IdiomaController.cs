...
namespace CollectorsClub.Web.API.Controllers {
	...
	[Authorize]
	public partial class CategoriaFoto_IdiomaController : ApiController {
		private readonly ICommandBus commandBus;
		private readonly ICategoriaFoto_IdiomaRepository categoriafoto_idiomaRepository;
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		
		public CategoriaFoto_IdiomaController(ICommandBus commandBus, ICategoriaFoto_IdiomaRepository categoriafoto_idiomaRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<CategoriaFoto_Idioma>>(HttpStatusCode.OK, categoriafoto_idiomaRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				CategoriaFoto_Idioma categoriafoto_idioma = categoriafoto_idiomaRepository.GetById(include, (p => p.Id == Id));
				if (categoriafoto_idioma == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
				return Request.CreateResponse<CategoriaFoto_Idioma>(HttpStatusCode.OK, categoriafoto_idioma, formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<CategoriaFoto_Idioma>>(HttpStatusCode.OK, categoriafoto_idiomaRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, categoriafoto_idiomaRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Post(CategoriaFoto_IdiomaModel categoriafoto_idioma) {
			try {
				if (ModelState.IsValid) {
					if (categoriafoto_idioma.Id == 0) {
						var command = AutoMapper.Mapper.Map<CategoriaFoto_IdiomaModel, CreateOrUpdateCategoriaFoto_IdiomaCommand>(categoriafoto_idioma);
						var result = commandBus.Submit(command);
						if (result.Success) {
							categoriafoto_idioma = AutoMapper.Mapper.Map<CreateOrUpdateCategoriaFoto_IdiomaCommand, CategoriaFoto_IdiomaModel>(command);
							var response = Request.CreateResponse<CategoriaFoto_IdiomaModel>(HttpStatusCode.Created, categoriafoto_idioma);
							string uri = Url.Route(null, new { Id = categoriafoto_idioma.Id });
		...
		public HttpResponseMessage Put(int Id, CategoriaFoto_IdiomaModel categoriafoto_idioma) {
			try {
				if (ModelState.IsValid) {
					var command = AutoMapper.Mapper.Map<CategoriaFoto_IdiomaModel, CreateOrUpdateCategoriaFoto_IdiomaCommand>(categoriafoto_idioma);
					var result = commandBus.Submit(command);
					return Request.CreateResponse<CategoriaFoto_IdiomaModel>(HttpStatusCode.OK, categoriafoto_idioma);
				} else {
					var errors = new Dictionary<string, IEnumerable<string>>();
					foreach (var keyValue in ModelState) {
							errors[keyValue.Key] = keyValue.Value.Errors.Select(e => (!string.IsNullOrWhiteSpace(e.ErrorMessage) ? e.ErrorMessage : (e.Exception != null ? e.Exception.Message : string.Empty)));
		...
		public HttpResponseMessage Delete(int Id) {
			try {
				var command = new DeleteCategoriaFoto_IdiomaCommand(Id);
				var result = commandBus.Submit(command);
