...
namespace CollectorsClub.Web.API.Controllers {
	...
	[Authorize]
	public partial class CategoriaCalendario_IdiomaController : ApiController {
		private readonly ICommandBus commandBus;
		private readonly ICategoriaCalendario_IdiomaRepository categoriacalendario_idiomaRepository;
	  private static readonly ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
		
		public CategoriaCalendario_IdiomaController(ICommandBus commandBus, ICategoriaCalendario_IdiomaRepository categoriacalendario_idiomaRepository) {
		...
		public HttpResponseMessage Get([FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IQueryable<CategoriaCalendario_Idioma>>(HttpStatusCode.OK, categoriacalendario_idiomaRepository.GetAll(include).AsQueryable(), formatter);
		...
		public HttpResponseMessage Get([FromUri] int Id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				CategoriaCalendario_Idioma categoriacalendario_idioma = categoriacalendario_idiomaRepository.GetById(include, (p => p.Id == Id));
				if (categoriacalendario_idioma == null) { throw new HttpResponseException(HttpStatusCode.NotFound); }
				return Request.CreateResponse<CategoriaCalendario_Idioma>(HttpStatusCode.OK, categoriacalendario_idioma, formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage GetList([FromUri] int[] id, [FromUri] string[] include, [FromUri] bool indent = false) {
			try {
				...
				return Request.CreateResponse<IEnumerable<CategoriaCalendario_Idioma>>(HttpStatusCode.OK, categoriacalendario_idiomaRepository.GetMany(include, (p => id.Contains(p.Id))), formatter);
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Count() {
			try {
				return Request.CreateResponse(HttpStatusCode.OK, categoriacalendario_idiomaRepository.Count());
			} catch (Exception _excepcion) {
				log.Error(_excepcion);
				return Request.CreateResponse(HttpStatusCode.InternalServerError, _excepcion);
			}
		...
		public HttpResponseMessage Post(CategoriaCalendario_IdiomaModel categoriacalendario_idioma) {
			try {
				if (ModelState.IsValid) {
					if (categoriacalendario_idioma.Id == 0) {
						var command = AutoMapper.Mapper.Map<CategoriaCalendario_IdiomaModel, CreateOrUpdateCategoriaCalendario_IdiomaCommand>(categoriacalendario_idioma);
						var result = commandBus.Submit(command);
						if (result.Success) {
							categoriacalendario_idioma = AutoMapper.Mapper.Map<CreateOrUpdateCategoriaCalendario_IdiomaCommand, CategoriaCalendario_IdiomaModel>(command);
							var response = Request.CreateResponse<CategoriaCalendario_IdiomaModel>(HttpStatusCode.Created, categoriacalendario_idioma);
							string uri = Url.Route(null, new { Id = categoriacalendario_idioma.Id });
		...
		public HttpResponseMessage Put(int Id, CategoriaCalendario_IdiomaModel categoriacalendario_idioma) {
			try {
				if (ModelState.IsValid) {
					var command = AutoMapper.Mapper.Map<CategoriaCalendario_IdiomaModel, CreateOrUpdateCategoriaCalendario_IdiomaCommand>(categoriacalendario_idioma);
					var result = commandBus.Submit(command);
					return Request.CreateResponse<CategoriaCalendario_IdiomaModel>(HttpStatusCode.OK, categoriacalendario_idioma);
				} else {
					var errors = new Dictionary<string, IEnumerable<string>>();
					foreach (var keyValue in ModelState) {
							errors[keyValue.Key] = keyValue.Value.Errors.Select(e => (!string.IsNullOrWhiteSpace(e.ErrorMessage) ? e.ErrorMessage : (e.Exception != null ? e.Exception.Message : string.Empty)));
		...
		public HttpResponseMessage Delete(int Id) {
			try {
				var command = new DeleteCategoriaCalendario_IdiomaCommand(Id);
				var result = commandBus.Submit(command);
