(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/UsuarioModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/UsuarioModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/UsuarioModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.UsuarioController = factory(jQuery, window.GlobalBaseController, UsuarioModel);
	...
	return function UsuarioController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'Usuarios';
		self.settings.EntityName = 'Usuario';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'Nombre': 'Nombre' + i, 'PrimerApellido': 'PrimerApellido' + i, 'SegundoApellido': 'SegundoApellido' + i, 'NombreDeUsuario': 'NombreDeUsuario' + i, 'CorreoElectronico': 'CorreoElectronico' + i, 'IdMarca': 'IdMarca' + i });
		}
