(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/TipoColeccionCalendarioModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/TipoColeccionCalendarioModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/TipoColeccionCalendarioModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.TipoColeccionCalendarioController = factory(jQuery, window.GlobalBaseController, TipoColeccionCalendarioModel);
	...
	return function TipoColeccionCalendarioController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'TiposColeccionCalendario';
		self.settings.EntityName = 'TipoColeccionCalendario';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'Nombre': 'Nombre' + i, 'Codigo': 'Codigo' + i, 'IdMarca': 'IdMarca' + i });
		}
		...
		//baseController.DescargarScripts('TipoColeccionCalendario_Idioma', settings);
