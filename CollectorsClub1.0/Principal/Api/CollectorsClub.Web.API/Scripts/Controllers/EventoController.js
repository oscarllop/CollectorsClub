(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/EventoModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/EventoModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/EventoModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.EventoController = factory(jQuery, window.GlobalBaseController, EventoModel);
	...
	return function EventoController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'Eventos';
		self.settings.EntityName = 'Evento';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'FechaAlta': (typeof moment !== 'undefined' ? moment().format() : new Date('01/01/0001 0:00:00')), 'FechaUltimaModificacion': null, 'Fecha': (typeof moment !== 'undefined' ? moment().format() : new Date('01/01/0001 0:00:00')), 'HoraInicio': (typeof moment !== 'undefined' ? moment().format() : new Date('00:00:00')), 'HoraFin': (typeof moment !== 'undefined' ? moment().format() : new Date('00:00:00')), 'IdTipo': i, 'Activa': false, 'IdMarca': 'IdMarca' + i, 'Nombre': 'Nombre' + i, 'Descripcion': null, 'Ubicacion': 'Ubicacion' + i });
		}
		...
		//baseController.DescargarScripts('Evento_Idioma', settings);
