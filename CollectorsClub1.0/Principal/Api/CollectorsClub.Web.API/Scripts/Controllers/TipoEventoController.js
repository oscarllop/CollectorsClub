(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/TipoEventoModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/TipoEventoModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/TipoEventoModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.TipoEventoController = factory(jQuery, window.GlobalBaseController, TipoEventoModel);
	...
	return function TipoEventoController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'TiposEvento';
		self.settings.EntityName = 'TipoEvento';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'Nombre': 'Nombre' + i, 'Codigo': 'Codigo' + i, 'Orden': i, 'IdMarca': 'IdMarca' + i });
		}
		...
		//baseController.DescargarScripts('TipoEvento_Idioma', settings);
