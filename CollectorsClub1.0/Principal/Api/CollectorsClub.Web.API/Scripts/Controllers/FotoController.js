(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/FotoModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/FotoModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/FotoModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.FotoController = factory(jQuery, window.GlobalBaseController, FotoModel);
	...
	return function FotoController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'Fotos';
		self.settings.EntityName = 'Foto';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'FechaAlta': (typeof moment !== 'undefined' ? moment().format() : new Date('01/01/0001 0:00:00')), 'FechaUltimaModificacion': null, 'NombreArchivoImagen': 'NombreArchivoImagen' + i, 'Orden': i, 'Activa': false, 'IdCategoria': i, 'Nombre': 'Nombre' + i, 'Descripcion': 'Descripcion' + i, 'IdMarca': 'IdMarca' + i });
		}
		...
		//baseController.DescargarScripts('Foto_Idioma', settings);
