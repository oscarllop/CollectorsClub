(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/SubcategoriaCalendario_IdiomaModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/SubcategoriaCalendario_IdiomaModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/SubcategoriaCalendario_IdiomaModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.SubcategoriaCalendario_IdiomaController = factory(jQuery, window.GlobalBaseController, SubcategoriaCalendario_IdiomaModel);
	...
	return function SubcategoriaCalendario_IdiomaController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'SubcategoriasCalendario_Idiomas';
		self.settings.EntityName = 'SubcategoriaCalendario_Idioma';
