(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/TipoColeccionCalendario_IdiomaModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/TipoColeccionCalendario_IdiomaModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/TipoColeccionCalendario_IdiomaModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.TipoColeccionCalendario_IdiomaController = factory(jQuery, window.GlobalBaseController, TipoColeccionCalendario_IdiomaModel);
	...
	return function TipoColeccionCalendario_IdiomaController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'TiposColeccionCalendario_Idiomas';
		self.settings.EntityName = 'TipoColeccionCalendario_Idioma';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'IdRegistro': i, 'Cultura': 'Cultura' + i, 'Nombre': 'Nombre' + i });
		}
