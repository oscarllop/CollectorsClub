(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/BaseController', 'WebAPI/Models/Evento_IdiomaModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/BaseController'), require('WebAPI/Models/Evento_IdiomaModel'));
	} else {
		// Browser globals
		if (typeof (BaseController) == 'undefined') {
			$.when(
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/BaseController.js'),
				$.getScript(parametrosGlobales.UrlWebAPI + '/scripts/Models/Evento_IdiomaModel.js'),
				$.Deferred(function (deferred) { $(deferred.resolve); })
			).done(function () {
				window.Evento_IdiomaController = factory(jQuery, window.GlobalBaseController, Evento_IdiomaModel);
	...
	return function Evento_IdiomaController(settings) {
		var self = this;
		self.settings = $.extend({}, settings);
		self.settings.EntitySet = 'Eventos_Idiomas';
		self.settings.EntityName = 'Evento_Idioma';
		...
		for (i = 1; i <= 15; i++) {
			self.registrosPrueba.push({ 'Id': i, 'IdRegistro': i, 'Cultura': 'Cultura' + i, 'Ubicacion': 'Ubicacion' + i, 'Nombre': 'Nombre' + i, 'Descripcion': 'Descripcion' + i });
		}
