(function (factory) {
	if (typeof define === 'function' && define.amd) {
		...
		window.CategoriaCalendarioModel = factory(jQuery, MarcaModel);
	}
}(function ($, moment, MarcaModel) {
	function CategoriaCalendarioModel(item, traducciones) {
		...
		this.Subcategorias = ko.observableArray();
	...
	CategoriaCalendarioModel.prototype.UpdateData = function (item, traducciones) {
		ParsedModels.put(item, this);
		this.Id(item.Id || 0);
		this.Nombre(item.Nombre || '');
		this.Codigo(item.Codigo || '');
		this.IdMarca(item.IdMarca || null);
		this.Subcategorias(typeof (item) !== 'undefined' && typeof (item.Subcategorias) === 'object' && item.Subcategorias !== null ? $.map(item.Subcategorias, function (registro) { if (!ParsedModels.containsKey(registro)) { return new SubcategoriaCalendarioModel(registro, traducciones); } else { return ParsedModels.get(registro); } }) : []);
		this.RegistrosIdiomas(typeof (item) !== 'undefined' && typeof (item.RegistrosIdiomas) === 'object' && item.RegistrosIdiomas !== null ? $.map(item.RegistrosIdiomas, function (registro) { if (!ParsedModels.containsKey(registro)) { return new CategoriaCalendario_IdiomaModel(registro, traducciones); } else { return ParsedModels.get(registro); } }) : []);
	...
	return CategoriaCalendarioModel;
}));
