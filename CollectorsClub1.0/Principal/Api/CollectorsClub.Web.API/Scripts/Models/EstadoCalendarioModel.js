(function (factory) {
	if (typeof define === 'function' && define.amd) {
		...
		window.EstadoCalendarioModel = factory(jQuery, MarcaModel);
	}
}(function ($, moment, MarcaModel) {
	function EstadoCalendarioModel(item, traducciones) {
	...
	EstadoCalendarioModel.prototype.UpdateData = function (item, traducciones) {
		...
		this.RegistrosIdiomas(typeof (item) !== 'undefined' && typeof (item.RegistrosIdiomas) === 'object' && item.RegistrosIdiomas !== null ? $.map(item.RegistrosIdiomas, function (registro) { if (!ParsedModels.containsKey(registro)) { return new EstadoCalendario_IdiomaModel(registro, traducciones); } else { return ParsedModels.get(registro); } }) : []);
