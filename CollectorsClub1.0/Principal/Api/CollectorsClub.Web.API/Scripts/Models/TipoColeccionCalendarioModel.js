(function (factory) {
	if (typeof define === 'function' && define.amd) {
		...
		window.TipoColeccionCalendarioModel = factory(jQuery, MarcaModel);
	}
}(function ($, moment, MarcaModel) {
	function TipoColeccionCalendarioModel(item, traducciones) {
	...
	TipoColeccionCalendarioModel.prototype.UpdateData = function (item, traducciones) {
		...
		this.RegistrosIdiomas(typeof (item) !== 'undefined' && typeof (item.RegistrosIdiomas) === 'object' && item.RegistrosIdiomas !== null ? $.map(item.RegistrosIdiomas, function (registro) { if (!ParsedModels.containsKey(registro)) { return new TipoColeccionCalendario_IdiomaModel(registro, traducciones); } else { return ParsedModels.get(registro); } }) : []);
	}

	return TipoColeccionCalendarioModel;
}));
