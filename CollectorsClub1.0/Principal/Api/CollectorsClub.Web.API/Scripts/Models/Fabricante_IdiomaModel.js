(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'moment/moment-with-locales.min', 'WebAPI/Models/FabricanteModel'], factory);
	} else if (typeof exports === 'object') {
		// CommonJS
		factory(require('jquery'), require('moment/moment-with-locales.min'), require('WebAPI/Models/FabricanteModel'));
	} else {
		window.Fabricante_IdiomaModel = factory(jQuery, FabricanteModel);
	}
}(function ($, moment, FabricanteModel) {
	function Fabricante_IdiomaModel(item, traducciones) {
	...
	Fabricante_IdiomaModel.prototype.UpdateData = function (item, traducciones) {
		...
		if (typeof (item) !== 'undefined' && typeof (item.Registro) === 'object' && item.Registro !== null) {
			if (!ParsedModels.containsKey(item.Registro)) {
				this.Registro(new FabricanteModel(item.Registro, traducciones));
			} else {
