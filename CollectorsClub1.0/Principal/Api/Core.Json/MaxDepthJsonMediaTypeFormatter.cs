...
namespace Core.Json {
	...
	/// </summary>
	public class MaxDepthJsonMediaTypeFormatter : JsonMediaTypeFormatter {
		...
		///// </summary>
		//public MaxDepthJsonMediaTypeFormatter() {
		...
		public override Task WriteToStreamAsync(Type type, object value, Stream writeStream, HttpContent content, TransportContext transportContext) {
			...
			if (UseDataContractJsonSerializer && Indent) {
				throw new NotSupportedException(string.Format("Indentation is not supported by '{0}'", typeof(DataContractJsonSerializer)));
			}
			...
			if (!UseDataContractJsonSerializer) {
				return TaskHelpers.RunSynchronously(() => {
					...
					using (CustomJsonTextWriter jsonTextWriter = new CustomJsonTextWriter(new StreamWriter(writeStream, effectiveEncoding)) { CloseOutput = false }) {
						if (Indent) {
							jsonTextWriter.Formatting = Newtonsoft.Json.Formatting.Indented;
						}
						JsonSerializer jsonSerializer = JsonSerializer.Create(base.SerializerSettings);
						Func<MemberInfo, bool> include = (m) => jsonTextWriter.CurrentDepth < jsonSerializer.MaxDepth || jsonTextWriter.CurrentDepth == jsonSerializer.MaxDepth && !((PropertyInfo) m).PropertyType.FullName.Contains("Model.Entities");
						//Func<MemberInfo, bool> include = (m) => jsonTextWriter.CurrentDepth < jsonSerializer.MaxDepth;
						if (jsonSerializer.ContractResolver != null) { jsonSerializer.ContractResolver = new CustomContractResolver(include); }
		...
		//	DataContractJsonSerializer serializer = _dataContractSerializerCache.GetOrAdd(type, (t) => CreateDataContractSerializer(type));

		//	if (serializer == null) {
		//		// A null serializer means the type cannot be serialized
		//		throw new InvalidOperationException(
		//				string.Format("The '{0}' serializer cannot serialize the type '{1}'.", typeof(DataContractJsonSerializer).Name, type.Name));
		//	}
